...
def check_content_categories():
    ...
    try:
        ...
        return category_counts, platform_counts
...
def validate_categories():
    ...
    try:
        ...
        if invalid_content:
            print(f"\nFound {len(invalid_content)} items with invalid categories:")
            for content in invalid_content[:5]:
                try:
                    title_safe = content.title[:50].encode('ascii', 'ignore').decode('ascii')
                    print(f"  ID {content.id}: '{title_safe}...' - Category: '{content.category}'")
                except:
                    print(f"  ID {content.id}: [Title with special chars] - Category: '{content.category}'")
